...
class MemeEngine():
    ...
    Main methods:
        make_meme(...) -- Create meme from image, text and author.
    """

    def __init__(self, base_path: str = './static',
                 fnt_path: str = './fonts/LilitaOne-Regular.ttf',
                 fnt_size: int = 40, max_words: int = 6,
                 padding: float = 0.05, gap: int = 10):
        """
        ...
        Arguments:
            base_path: str -- Base path to save memes
            fnt_path: str -- Path to font
            fnt_size: str -- Maximum font size for quote (might get reduced)
            max_words: str -- Maximum number of words per line for quote
            padding: str -- Padding from image boarder as a fraction of width
            gap: str -- Gap between quote body and author in pixels
        """
    ...
    def prepare_quote(self, body: str, author: str) -> None:
        ...
        - Calls make_multiline() to break up long quotes into multiple lines.
        - Calls resize_fonts() to reduce font sizes until the quote fits.
        """
    ...
    def add_quote(self) -> None:
        ...
        - Calls self.random_location() to get valid random coordinates.
        """
        d = ImageDraw.Draw(self.image)
        x_body, y_body, x_auth, y_auth = self.random_location()
        d.text((x_body, y_body),
               self.str_body, font=self.fnt_body, fill='white')
        d.text((x_auth, y_auth),
               self.str_auth, font=self.fnt_auth, fill='white')
    ...
    def resize_fonts(self) -> None:
        ...
        while True:
            ...
            _, _, w_auth, h_auth = d.textbbox((0, 0),
                                              text=self.str_auth,
                                              font=self.fnt_auth)
            if ((max(w_body, w_auth) <
                 (1-2*self.padding)*self.image.size[0])
                and (h_body+h_auth+self.gap <
                     (1-2*self.padding)*self.image.size[1])):
                break
            else:
                self.fnt_size -= 1
    ...
    def random_location(self) -> Tuple[int, int, int, int]:
        """
        Create coordinates for quote body and author at random location.

        Returns:
            x_body, y_body, x_auth, y_auth -- coordinates of the quote
        """
        d = ImageDraw.Draw(self.image)
        _, _, w_body, h_body = d.textbbox((0, 0),
                                          text=self.str_body,
